...
O grafo oficial de entrada deste trabalho é o representado na [figura 1](./imgs/graph.png), baseada no material discutido em [instrucoes.md](./instrucoes.md). A ideia central é separar o trabalho em duas partes:

1. uma **parte manual**, em que o aluno analisa o grafo original, identifica os vértices desbalanceados e realiza a adição de arestas necessárias, alterando lista de arestas de entrada para obter um grafo eulerizado;
...
- uso e adaptação da implementação-base do **método de Hierholzer**, como `DirectedEulerianCycle`, para obtenção do circuito euleriano, complementando-a quando necessário para considerar pesos e informar o custo total.
...
O papel conceitual desses algoritmos será reproduzido **manualmente** na construção do arquivo `entrada_eulerizada.txt`, por meio:

- da identificação dos vértices desbalanceados;
- da escolha dos caminhos a serem repetidos; e
- da adição das arestas necessárias para eulerizar o grafo.
...
<img src="imgs/graph.png" width="420">
...
- custo total igual a `10` (acompanhe o circuito pelas arestas do grafo eulerizado).

<img src="imgs/grafo_exemplo.png" width="200">

Figura 2. Dígrafo de exemplo original utilizado para validar formato de entrada e balanceamento. Fonte: visualização adaptada de <https://algorithms.discrete.ma.tum.de/graph-algorithms/directed-chinese-postman/index_en.html>

<img src="imgs/grafo_exemplo_eulerizado.png" width="200">

Figura 3. Dígrafo de exemplo após a eulerização manual, usado para validar o circuito euleriano e o custo total. Fonte: visualização adaptada de <https://algorithms.discrete.ma.tum.de/graph-algorithms/directed-chinese-postman/index_en.html>
...
- usar como referência `algs4-py` no caso de Python e `algs4-java` no caso de Java;
...
- A classe `DirectedEulerianCycle` retorna o circuito euleriano como sequência de vértices, pois a existência de circuito euleriano é uma propriedade estrutural do dígrafo, independente dos pesos.
- Caso o aluno adapte `DirectedEulerianCycle` para trabalhar diretamente com arestas ponderadas e também retornar o custo do circuito, pode usar como referência adicional `EdgeWeightedDigraph.java` ou `edge_weighted_digraph.py`.
