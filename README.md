# T290 — Resolução de Problemas com Grafos

Material didático, implementações, datasets e trabalhos da disciplina.

## Comece aqui

1. Confira as [turmas e os horários](mat-didatico/ensino/turmas_2026_2.md).
2. Abra o cronograma correspondente: [turmas 09/19 e 62/63 — segundas e quartas](mat-didatico/ensino/cronograma_2026_2_turmas_09-19_62-63.md) ou [turmas 16/17 e 30/31 — terças e quintas](mat-didatico/ensino/cronograma_2026_2_turmas_16-17_30-31.md).
3. Consulte o plano resumido da sua turma para lançamento no AVA: [turmas 09/19 e 62/63](mat-didatico/ensino/plano_ensino_2026_2_turmas_09-19_62-63.md) ou [turmas 16/17 e 30/31](mat-didatico/ensino/plano_ensino_2026_2_turmas_16-17_30-31.md).
4. Escolha a linguagem e execute o primeiro exemplo: [Python](algs4-py/guia_pratico.md) ou [Java](algs4-java/guia_pratico.md).
5. Consulte o [índice dos trabalhos práticos](mat-didatico/trabalhos/README.md).

## Disciplina e calendário

- [Projeto de ensino 2026.2](mat-didatico/ensino/proj_ensino_2026.md)
- [Calendário acadêmico 2026.2](mat-didatico/ensino/calendario_2026_2.md)
- [Turmas e horários](mat-didatico/ensino/turmas_2026_2.md)
- Planos resumidos: [turmas 09/19 e 62/63](mat-didatico/ensino/plano_ensino_2026_2_turmas_09-19_62-63.md) e [turmas 16/17 e 30/31](mat-didatico/ensino/plano_ensino_2026_2_turmas_16-17_30-31.md)

## Materiais de ensino

- [Aulas da disciplina](mat-didatico/aulas/)
- [Materiais EAD](mat-didatico/EAD/)
- [Recursos visuais](mat-didatico/recursos/visuais.md)
- [Referências adicionais](mat-didatico/recursos/refs.md)
- [Plataformas de exercícios](mat-didatico/ensino/plataformas.md)

## Implementações e execução

- [Python — visão geral](algs4-py/README.md)
- [Python — guia prático](algs4-py/guia_pratico.md)
- [Java — visão geral](algs4-java/README.md)
- [Java — guia prático](algs4-java/guia_pratico.md)
- [Implementações Java relacionadas ao cronograma](mat-didatico/ensino/implementacoes_algs4_java.md)

Os algoritmos são apresentados por construção guiada. Os grupos deverão compreender, rastrear, integrar e adaptar as implementações de referência ao problema atribuído.

## Trabalhos

- [Índice dos trabalhos práticos](mat-didatico/trabalhos/README.md)
- [T1 — Modelagem, representação e DFS/BFS](mat-didatico/trabalhos/T1.md)
- [T2 — Conectividade e propriedades estruturais](mat-didatico/trabalhos/T2.md)
- [T3 — Otimização em grafos](mat-didatico/trabalhos/T3.md)
- [Template institucional](mat-didatico/trabalhos/template/)

## Exercícios complementares

- [Exercícios e respostas para estudo autônomo](mat-didatico/exercicios/)

As respostas são materiais de apoio ao estudo e não constituem trabalhos avaliativos.

## Datasets

- [Conjuntos de dados](dataset/)
- [Instruções para baixar arquivos grandes](dataset/README.md)

## Estrutura do repositório

- `mat-didatico/`: projeto de ensino, cronogramas e materiais didáticos.
- `algs4-py/`: implementações de referência em Python.
- `algs4-java/`: implementações de referência em Java.
- `dataset/`: dados usados em exemplos, testes e exercícios.
- `mat-didatico/trabalhos/`: avaliações práticas, acompanhamentos e apresentação.
- `mat-didatico/exercicios/`: exercícios complementares com respostas para estudo.
