<img src="../../../imgs/UNIFOR_logo1b.png" width="400"> 

# Resolução de Problemas com Grafos
Orientador: Prof. Me Ricardo Carubbi

# Trabalho Prático 2

Esta atividade vale `1,0 ponto`.

## Tema

Implemente, em Python ou em Java, um programa que trabalhe com o grafo do cavalo em um tabuleiro de xadrez `3 x 3`.

Cada casa do tabuleiro deve ser representada por um vértice. Duas casas devem ser ligadas por uma aresta sempre que um cavalo puder sair de uma delas e chegar à outra em um único movimento.

<img src="../../../imgs/tab_cavalo.png" width="200">

Figura 1. Exemplo de movimento do cavalo em um tabuleiro de xadrez.

A numeração dos vértices deve ser feita na ordem de leitura do tabuleiro, da esquerda para a direita e de cima para baixo.

Cada aluno ou dupla deverá construir manualmente a lista de arestas desse grafo, no formato utilizado em `algs4`.

## Objetivo

Aplicar os conceitos de modelagem em grafos, componentes conexas e análise estrutural em um problema baseado nos movimentos do cavalo em um tabuleiro de xadrez.

## Entrada

O programa deve ler uma lista de arestas no formato `algs4`:

- número de vértices `V`
- número de arestas `E`
- `E` linhas com pares de vértices `v w`

Para este trabalho, o arquivo de entrada deverá representar o grafo do cavalo em um tabuleiro `3 x 3`.

## Respostas esperadas

Para o exemplo adotado neste trabalho, as respostas esperadas são:

- Lista de adjacência:

```text
0: 7 5
1: 8 6
2: 7 3
3: 8 2
4:
5: 6 0
6: 5 1
7: 2 0
8: 3 1
```

- Componentes conexas: `2`
- Vértices da componente `0`: `0 1 2 3 5 6 7 8`
- Vértices da componente `1`: `4`
- O grafo possui ciclo: `Sim`
- Um ciclo encontrado: `0 5 6 1 8 3 2 7 0`

## Perguntas que o programa deve responder

- Qual é o grafo do cavalo informado, na forma de lista de adjacência?
- Quais são as componentes conexas do grafo?
- O grafo possui ciclo?
- Se o grafo possuir ciclo, quais são os vértices de um ciclo encontrado?

## Pontuação

- Imprimir o grafo na forma de lista de adjacência: `0,30`
- Identificar as componentes conexas do grafo: `0,30`
- Indicar se o grafo possui ciclo: `0,20`
- Informar os vértices de um ciclo encontrado, quando existir: `0,20`

## Critérios de avaliação

- Correção da modelagem do tabuleiro como grafo.
- Correção da construção das arestas a partir dos movimentos do cavalo.
- Uso adequado de algoritmos para identificar componentes conexas.
- Correção da análise sobre existência de ciclos.
- Clareza na exibição dos resultados.

## Requisitos

- Implementar a solução em Python ou em Java.
- Representar o grafo por lista de adjacência.
- Construir manualmente a lista de arestas do grafo do cavalo `3 x 3`.
- Numerar os vértices na ordem de leitura do tabuleiro.
- Exibir os resultados de forma clara.

## Estrutura esperada do projeto

```text
t2-cavalo/
├── README.md
├── dados/
│   └── entrada.txt
└── src/
    ├── Main.java | main.py
    ├── Graph.java | graph.py (incluir dependências)
    ├── CC.java | cc.py
    ├── Cycle.java | cycle.py
    └── demais classes auxiliares utilizadas
```

## Observações

- A estrutura do projeto apresentada neste enunciado é obrigatória.
- A entrega deverá ser feita por meio de apenas um link do GitHub no AVA/Moodle.
- O arquivo `Main.java` ou `main.py` deverá ser o ponto de entrada do programa e deverá gerar as respostas solicitadas neste trabalho.
- Na implementação em Python, a classe de detecção de ciclo poderá precisar ser adaptada para também informar os vértices de um ciclo encontrado. Em Java, essa funcionalidade já existe na versão padrão.
